...
| Stock receiving | `StockEntryViewSet` | A stock-entry POST records the actor and atomically increases inventory. |
| Selling | `SaleViewSet` | A sale locks inventory rows, validates available stock, snapshots item prices, decrements stock, and records the cashier in one transaction. |
| Repairs | `RepairViewSet` | Creates a generated ticket linked to the actor. |
...
- Inventory has a direct PATCH endpoint for operational corrections. Those edits are not represented by `StockEntry`; introduce a dedicated stock-adjustment model before relying on adjustment audit trails.
